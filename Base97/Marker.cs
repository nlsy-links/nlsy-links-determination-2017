...
namespace Nls.Base97 {
    public sealed class Marker {
        ...
        private readonly Item[] _items = { Item.roster_relationship_1_dim};//, Item.IDCodeOfOtherSiblingGen1, Item.ShareBiomomGen1, Item.ShareBiodadGen1 };
        ...
        #endregion
        #region Constructor
        public Marker( LinksDataSet dsLinks, ImportDataSet dsImport ) {
            ...
            if( dsLinks.tblMarker.Count != 0 ) throw new ArgumentException("There should be zero rows in tblMarker.");
            _dsLinks = dsLinks;
            _dsImport = dsImport;
        ...
        private void AddMarkerRow( Int16 extendedID, Int32 relatedID, MarkerType markerType, Int16 surveyYear, MarkerEvidence mzEvidence, MarkerEvidence sameGenerationEvidence, MarkerEvidence biomomEvidence, MarkerEvidence biodadEvidence, MarkerEvidence biograndparentEvidence ) {
            LinksDataSet.tblMarkerRow drNew = _dsLinks.tblMarker.NewtblMarkerRow();
            ...
            _dsLinks.tblMarker.AddtblMarkerRow(drNew);
        }
        #endregion
        #region Public Static Methods
        //internal static MarkerEvidence RetrieveParentMarkerSingleYear( Int64 relatedIDLeft, MarkerType markerType, Bioparent bioparent, LinksDataSet.tblMarkerDataTable dtMarker ) {
        ...
        //    LinksDataSet.tblMarkerRow[] drs = (LinksDataSet.tblMarkerRow[])dtMarker.Select(select, sort);
        ...
        //}
        //internal static MarkerEvidence RetrieveParentMarkerMultiYear( Int64 relatedIDLeft, MarkerType markerType, Int16 year, Bioparent bioparent, LinksDataSet.tblMarkerDataTable dtMarker ) {
        //    if( dtMarker == null ) throw new ArgumentNullException("dtMarker");
        ...
        //        throw new ArgumentOutOfRangeException("markerType", markerType, "The 'bioparent' value is not accepted by this function.");
        //}
        internal static MarkerSummary[] RetrieveMarkers( Int64 relatedIDLeft, MarkerType markerType, LinksDataSet.tblMarkerDataTable dtMarker, Int32 maxCount ) {
            if( dtMarker == null ) throw new ArgumentNullException("dtMarker");
            string select = string.Format("{0}={1} AND {2}={3}",
                relatedIDLeft, dtMarker.RelatedIDColumn.ColumnName,
                (byte)markerType, dtMarker.MarkerTypeColumn.ColumnName);
            LinksDataSet.tblMarkerRow[] drs = (LinksDataSet.tblMarkerRow[])dtMarker.Select(select);
            Trace.Assert(drs.Length <= maxCount, "The number of returns markers should not exceed " + maxCount + ".");
            MarkerSummary[] evidences = new MarkerSummary[drs.Length];
            for( Int32 i = 0; i < drs.Length; i++ ) {
                evidences[i] = new MarkerSummary((
        ...
        internal static LinksDataSet.tblMarkerDataTable PairRelevantMarkerRows( Int64 relatedIDLeft, Int64 relatedIDRight, LinksDataSet dsLinks, Int32 extendedID ) {
            string select = string.Format("{0}={1} AND {2} IN ({3},{4})",
                extendedID, dsLinks.tblMarker.ExtendedIDColumn.ColumnName,
                dsLinks.tblMarker.RelatedIDColumn.ColumnName, relatedIDLeft, relatedIDRight);
            LinksDataSet.tblMarkerRow[] drs = (LinksDataSet.tblMarkerRow[])dsLinks.tblMarker.Select(select);
            //if ( drs.Length <= 0 ) {
            //   return null;
            //}
            //else {
            LinksDataSet.tblMarkerDataTable dt = new LinksDataSet.tblMarkerDataTable();
            foreach( LinksDataSet.tblMarkerRow dr in drs ) {
                dt.ImportRow(dr);
            }
            return dt;
        }
