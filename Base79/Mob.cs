		...
		internal static DateTime? Retrieve ( LinksDataSet.tblSubjectRow drSubject, LinksDataSet.tblResponseDataTable dt ) {
			...
			switch ( (Sample)drSubject.Generation ) {
				case Sample.Nlsy79Gen1:
					...
					Int32? monthGen1 = Nls.Base79.Retrieve.ResponseNullPossible(Constants.Gen1MobSurveyYearPreferred, Item.DateOfBirthMonth, sourceGen1, drSubject.SubjectTag, maxRows, dt);
					Int32 yearGen1 = Int32.MinValue;
					if ( monthGen1.HasValue && monthGen1.Value > 0 ) {
						yearGen1 = Nls.Base79.Retrieve.Response(Constants.Gen1MobSurveyYearPreferred, Item.DateOfBirthYearGen1, drSubject.SubjectTag, maxRows, dt);
						Trace.Assert(yearGen1 > 0, "If month isn't missing, neither should year.");
					}
					else {
						monthGen1 = Nls.Base79.Retrieve.Response(Constants.Gen1MobSurveyYearBackup, Item.DateOfBirthMonth, drSubject.SubjectTag, maxRows, dt);
						yearGen1 = Nls.Base79.Retrieve.Response(Constants.Gen1MobSurveyYearBackup, Item.DateOfBirthYearGen1, drSubject.SubjectTag, maxRows, dt);
					}
				...
				case Sample.Nlsy79Gen2:
					...
					Int32? monthGen2 = Nls.Base79.Retrieve.ResponseNullPossible(Item.DateOfBirthMonth, sourceGen2, drSubject.SubjectTag, maxRows, dt);
					Int32? yearGen2 = Nls.Base79.Retrieve.ResponseNullPossible(Item.DateOfBirthYearGen2, sourceGen2, drSubject.SubjectTag, maxRows, dt);//There's only three who don't have both year & month, so moving it won't increase speed.
