...
## Notes 
1. The current report covers `r nrow(ds)` tables.
1. `r scales::comma(nrow(ds_variable))` variables are defined, and `r scales::comma(sum(ds_variable$variable_active))` variables are 'active'.
...
## Tables
```{r table, echo=echo_chunks, message=message_chunks, warning=TRUE}
```

## Items
```{r item, echo=echo_chunks, message=message_chunks, warning=TRUE}
```

## Variables
```{r variable, echo=echo_chunks, message=message_chunks, warning=TRUE}
```
